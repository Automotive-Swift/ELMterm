...
- **Inferred CAN Frames**: Requests are annotated with the frame the adapter will put on the bus (e.g. `0100` → `7DF 02 01 00`), derived from the tracked protocol (`ATSP`/`ATTP`, `ATDP`/`ATDPN`, headered replies), header (`ATSH`, `ATCP`), `ATCAF` and `ATCEA`. Padding is omitted, matching how received frames are shown. The line (`On the bus (inferred): …`) is independent of `ATH0`/`ATH1`: those only change how the adapter displays *received* frames, while the request always carries its header on the bus. The line is only shown when the protocol is known to be ISO 15765-4 and the request fits a single frame — never guessed during automatic protocol search. While the protocol is still unknown, the first request carries a one-time hint to query it with `ATDPN`
